...
import { Copy, Download, Plus, X, ChevronLeft, ChevronRight, Clock, Settings, Target } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const convertScheduleForNewDuration = (newDuration: number): Schedule => {
    ...
    Object.entries(schedule).forEach(([key, task]) => {
      ...
      if (currentSlotIndex !== -1) {
        const timeInHours = timeConfig.startTime + (currentSlotIndex * timeConfig.duration);
        const newSlotIndex = Math.floor((timeInHours - timeConfig.startTime) / newDuration);
  ...
  const handleTimeConfigChange = (field: keyof TimeConfig, value: number): void => {
  ...
  return (
    <div className="flex h-screen bg-background">
      <div className={`${sidebarCollapsed ? 'w-16' : 'w-1/3'} transition-all duration-300 bg-card border-r shadow-lg overflow-hidden flex flex-col`}>
                ...
                <Tooltip
                  content={
                    <div>
                      ...
                      <p className="text-lg text-muted-foreground">還有 {calculateRemainingHours()} 小時的空閒時段</p>
                    </div>
                  }
                >
                  <div className="flex flex-col items-center space-y-1 cursor-help">
                    <Clock className="w-5 h-5 text-primary" />
                    <span className="text-xs font-bold text-primary">{calculateRemainingHours()}</span>
                  </div>
                </Tooltip>
              </div>
              ...
              <div className="text-center">
                      ...
                      <div className="space-y-3">
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => handleTimeConfigChange('startTime', parseInt(e.target.value))}
                            className="h-8"
                          />
                        </div>
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => handleTimeConfigChange('endTime', parseInt(e.target.value))}
                            className="h-8"
                          />
                        </div>
                        <div>
                          <label className="block text-sm font-medium mb-1">時段長度</label>
                          <Select value={timeConfig.duration.toString()} onValueChange={(value) => handleDurationChange(parseFloat(value))}>
                ...
                >
                  <div className="flex flex-col items-center space-y-1 cursor-pointer">
                    ...
                    <span className="text-xs">{timeConfig.duration === 0.5 ? '30m' : `${timeConfig.duration}h`}</span>
                  </div>
                </Tooltip>
              </div>
                    ...
                    <div className="w-72">
                        ...
                        {taskTypes.map(task => (
                          <div
                            ...
                            className={`flex items-center gap-2 p-2 rounded cursor-pointer hover:bg-muted/50 ${
                              selectedTask?.id === task.id ? 'bg-primary/10 ring-1 ring-primary' : ''
                            }`}
                      ...
                      {selectedTask && (
                        <div className="mt-3 pt-3 border-t">
                          <p className="text-xs text-muted-foreground">已選擇: <span className="font-semibold text-foreground">{selectedTask.name}</span></p>
                        </div>
                      )}
                      {!showAddTask && (
                        <Button
                ...
                >
                  <div className={`flex flex-col items-center space-y-1 cursor-pointer ${
                    ...
                    <div 
                      ...
                      style={{ backgroundColor: selectedTask?.color || '#94a3b8' }}
                    >
                      <Target className="w-3 h-3 text-white" />
                    </div>
                    <span className="text-xs">{selectedTask?.name.substring(0, 2) || '選擇'}</span>
                  </div>
                </Tooltip>
              </div>
            </div>
