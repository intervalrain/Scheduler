...
import { TaskType, TimeConfig, Schedule, DragState } from '../types';
...
interface UserContextType {
  ...
  setSprintStartDay: React.Dispatch<React.SetStateAction<number>>;
...
export const UserProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [sprintStartDay, setSprintStartDay] = useState<number>(() => {
    const saved = localStorage.getItem("sprintStartDay");
    return saved ? JSON.parse(saved) : 1; // Default to Monday
  });
  ...
  useEffect(() => {
    localStorage.setItem("sprintStartDay", JSON.stringify(sprintStartDay));
  }, [sprintStartDay]);
